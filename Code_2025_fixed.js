...
function createMasterSheet(ss, sheetName, offsetYear) {
    ...
    const currentFY = getDashboardFiscalYear(ss);
...
function 手動入力シート作成() {
    const ui = SpreadsheetApp.getUi();
    const result = ui.prompt('シート作成', '作成する年月を入力してください (例: 2026_04)', ui.ButtonSet.OK_CANCEL);
...
function getDashboardFiscalYear(ss) {
    // 1. ファイル名から取得を試みる
    const match = ss.getName().match(/_(\d{4})年度/);
    if (match) return parseInt(match[1], 10);

    // 2. 月別シート（YYYY_MM）が存在すれば、多数決で年度を判定する
    const sheets = ss.getSheets();
    const fyCounts = {};
    let maxCount = 0;
    let fallbackFy = null;
    ...
    for (let i = 0; i < sheets.length; i++) {
        const sm = sheets[i].getName().match(/^(\d{4})_(\d{2})$/);
        if (sm) {
            const y = parseInt(sm[1], 10);
            const m = parseInt(sm[2], 10);
            const sheetFy = getFiscalYear(new Date(y, m - 1, 1));
            fyCounts[sheetFy] = (fyCounts[sheetFy] || 0) + 1;

            if (fyCounts[sheetFy] > maxCount) {
                maxCount = fyCounts[sheetFy];
                fallbackFy = sheetFy;
            }
        }
    }
    ...
    if (fallbackFy !== null) {
        console.log('Deduced FY from sheets: ' + fallbackFy);
        return fallbackFy;
    }

    // 3. どちらも取得できなければ今日の日付から判定
...
function getOrCreateSpreadsheet() {
    try {
        ...
        if (activeSs) {
            console.log('Using active spreadsheet: ' + activeSs.getName());
            return activeSs;
        }
    } catch (e) {
